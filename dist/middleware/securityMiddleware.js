 ...
 */
export const checkFailedLoginAttempts = (email) => {
    const now = Date.now();
...
export const sanitizeBodyMiddleware = (req, res, next) => {
    ...
    for (const [key, value] of Object.entries(req.body)) {
        if (typeof value === 'string') {
            ...
            sanitizedBody[key] = value.map((v) => typeof v === 'string' ? sanitizeInput(v) : v);
        }
        else {
            sanitizedBody[key] = value;
        }
 ...
 * ペイロードサイズチェック
 */
export const validatePayloadSize = (maxSize = 10 * 1024) => {
    return (req, res, next) => {
...
export const encryptData = (data) => {
    ...
    const cipher = crypto.createCipheriv(ENCRYPTION_ALGORITHM, Buffer.from(ENCRYPTION_KEY.slice(0, 32).padEnd(32, '0')), iv);
...
export const decryptData = (encryptedData) => {
    ...
    const decipher = crypto.createDecipheriv(ENCRYPTION_ALGORITHM, Buffer.from(ENCRYPTION_KEY.slice(0, 32).padEnd(32, '0')), iv);
