...
// ============================================
// POST /api/users/register - ユーザー登録（メール＋パスワード＋名前＋年齢＋生年月日）
// ============================================
router.post('/register', async (req, res) => {
    try {
        const { email, password, name, age, birth_date } = req.body;
        // バリデーション
        if (!email || !password) {
        ...
        // ユーザーを登録（名前、年齢、生年月日を保存）
        const stmt = db.prepare(`INSERT INTO users (name, email, password, age, birth_date, gender, address, occupation, bio, status, progress_stage)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', 'birth')`);
        const result = stmt.run(name, email, hashedPassword, age, birth_date || null, null, null, null, null);
        // JWTトークンを生成
...
router.post('/login', async (req, res) => {
    try {
        ...
        // ユーザーを検索
        let user = db.prepare('SELECT id, email, password, name, age, birth_date FROM users WHERE email = ?').get(email);
        // ユーザーが存在しない場合は自動登録
        if (!user) {
            try {
                const hashedPassword = await hashPassword(password);
                ...
                const userBirthDate = birth_date || null;
                const stmt = db.prepare(`INSERT INTO users (name, email, password, age, birth_date, gender, address, occupation, bio, status, progress_stage)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', 'birth')`);
                const result = stmt.run(userName, email, hashedPassword, userAge, userBirthDate, null, null, null, null);
                user = {
                    id: result.lastInsertRowid,
                    email,
