...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const timelines = await queryAll(query, params);
...
router.post('/', authenticate, async (req, res) => {
    try {
        ...
        if (event_age !== undefined && event_age !== null) {
            try {
                const userRecord = await queryRow('SELECT birth_year FROM users WHERE id = ?', [user.userId]);
        ...
        // timeline テーブルに保存
        const result = await queryRun(`INSERT INTO timeline (user_id, age, year, month, turning_point, stage, event_title, event_description, edited_content, is_auto_generated)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id`, [
            user.userId,
            event_age ? parseInt(event_age) : null,
            event_year,
        ...
        const timelineId = result.rows?.[0]?.id;
      ...
      `, [user.userId, edited_content, edited_content, edited_content, edited_content]);
            console.log('✅ Biography saved - user_id:', user.userId);
        }
        // 写真データを保存
                ...
                for (let idx = 0; idx < answersWithPhotos.length; idx++) {
                    ...
                    if (answer.photos && Array.isArray(answer.photos) && answer.photos.length > 0) {
                        ...
                        for (let photoIdx = 0; photoIdx < answer.photos.length; photoIdx++) {
                            ...
                            await queryRun(`INSERT INTO timeline_photos (timeline_id, file_path, description, display_order, created_at)
                 VALUES (?, ?, ?, ?, NOW())`, [
                                timelineId,
                                photo.file_path,
        ...
        // 保存されたデータを取得して返す
        const savedTimeline = await queryRow('SELECT * FROM timeline WHERE id = ?', [timelineId]);
        res.status(201).json({
            success: true,
...
router.get('/user/:userId', authenticate, async (req, res) => {
    try {
        ...
        const timelineData = await queryAll(`
...
router.get('/:id', authenticate, async (req, res) => {
    try {
        ...
        const timeline = await queryRow('SELECT * FROM timeline WHERE id = ? AND user_id = ?', [id, user.userId]);
        if (!timeline) {
...
router.put('/:id', authenticate, async (req, res) => {
    try {
        ...
        const timeline = await queryRow('SELECT user_id FROM timeline WHERE id = ?', [id]);
        ...
        await queryRun(`
      UPDATE timeline 
      SET age = ?, year = ?, month = ?, turning_point = ?, stage = ?, event_title = ?, event_description = ?, edited_content = ?, is_auto_generated = ?, updated_at = NOW()
...
router.delete('/:id', authenticate, async (req, res) => {
    try {
        ...
        // timeline_photos も削除
        await queryRun('DELETE FROM timeline_photos WHERE timeline_id = ?', [id]);
        // timeline を削除
        await queryRun('DELETE FROM timeline WHERE id = ?', [id]);
...
router.get('/:timelineId/photos', authenticate, async (req, res) => {
    try {
        const { timelineId } = req.params;
        const user = req.user;
        console.log('📸 Get timeline photos - timelineId:', timelineId);
        // 本人確認（タイムラインの所有者確認）
        const timeline = await queryRow('SELECT user_id FROM timeline WHERE id = ?', [timelineId]);
        if (!timeline || timeline.user_id !== user.userId) {
...
router.post('/:timelineId/photos', authenticate, async (req, res) => {
    try {
        const { timelineId } = req.params;
        const user = req.user;
        ...
        `, [photoId, user.userId]);
          ...
          `, [timelineId, photo.file_path, photo.description, idx]);
                    insertedPhotos.push({
                        id: result.rows?.[0]?.id,
...
router.delete('/:timelineId/photos/:photoId', authenticate, async (req, res) => {
    try {
        ...
        await queryRun('DELETE FROM timeline_photos WHERE id = ? AND timeline_id = ?', [photoId, timelineId]);
...
// ============================================
router.post('/:timelineId/metadata', authenticate, async (req, res) => {
    try {
        const user = req.user;
        const { timelineId } = req.params;
        ...
        // timeline が存在するか確認
        const timeline = await queryRow('SELECT * FROM timeline WHERE id = ? AND user_id = ?', [timelineId, user.userId]);
        if (!timeline) {
            console.error('❌ Timeline not found');
            return res.status(404).json({ error: 'Timeline が見つかりません' });
        }
