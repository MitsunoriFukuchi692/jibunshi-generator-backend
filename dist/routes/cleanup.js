...
router.delete('/old-data', authenticate, async (req, res) => {
    try {
        const user = req.user;
        ...
        const biographyPhotosResult = await queryRun(`DELETE FROM biography_photos WHERE biography_id IN (SELECT id FROM biography WHERE user_id = ?)`, [user_id]);
        const biographyPhotosDeleted = biographyPhotosResult.rowCount || 0;
        console.log('  📸 biography_photos削除:', biographyPhotosDeleted, '件');
        const timelinePhotosResult = await queryRun(`DELETE FROM timeline_photos WHERE timeline_id IN (SELECT id FROM timeline WHERE user_id = ?)`, [user_id]);
