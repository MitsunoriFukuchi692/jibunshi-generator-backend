...
router.get('/users/:id/progress', (req, res) => {
    try {
        ...
        const photos = db.prepare('SELECT COUNT(*) as count FROM photos WHERE user_id = ?').get(id);
        const responses = db.prepare('SELECT COUNT(*) as count FROM responses WHERE user_id = ?').get(id);
        const pdfVersions = db.prepare('SELECT COUNT(*) as count FROM pdf_versions WHERE user_id = ?').get(id);
...
// ============================================
router.put('/users/:id/finalize', (req, res) => {
    try {
        const { id } = req.params;
