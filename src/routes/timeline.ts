...
router.post('/', authenticate, (req: Request, res: Response) => {
  try {
    ...
    const result = stmt.run(
      ...
      event_year,  // ✅ 修正3：year を event_year に変更
      ...
      edited_content || null,
      isAutoGenerated ? 1 : 0  // is_auto_generated: AI生成かどうか
    );
        ...
        for (let idx = 0; idx < answersWithPhotos.length; idx++) {
          ...
          if (answer.photos && Array.isArray(answer.photos) && answer.photos.length > 0) {
            ...
            for (let photoIdx = 0; photoIdx < answer.photos.length; photoIdx++) {
              ...
              photoStmt.run(
                timelineId,
                photo.file_path,  // base64 または URL パス
...
router.get('/:id', authenticate, (req: Request, res: Response) => {
  try {
    ...
    const timeline = stmt.get(id, user.userId) as any;
...
router.put('/:id', authenticate, (req: Request, res: Response) => {
  try {
    ...
    stmt.run(
      event_age ? parseInt(event_age) : null,
      event_year,  // ✅ 修正：year を event_year に変更
      month ? parseInt(month) : null,
      turning_point || null,
      stage || 'interview',
      event_title,
