...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    const stmt = db.prepare(
      `INSERT INTO pdf_versions (user_id, version, html_content, pdf_path, status)
       VALUES (?, 1, ?, ?, 'draft')`
    );
...
// ============================================
router.get('/versions/:userId', (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const stmt = db.prepare(
      'SELECT id, version, status, generated_at FROM pdf_versions WHERE user_id = ? ORDER BY version DESC'
    );
...
// ============================================
router.put('/:versionId', async (req: Request, res: Response) => {
  try {
...
// Helper: HTMLテンプレート生成
// ============================================
function generateHtmlTemplate(user: any): string {
